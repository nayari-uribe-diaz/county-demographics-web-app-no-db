...
def render_fact():
    ...
    counties = population_per_square_mile(state)
    fact = "In " + state + ", the county with the highest percentage of under 18 year olds is " + county + "."
    fact2 = "In " + state + ", " + counties + " has the highest population per square mile!"
...
def population_per_square_mile(state):
    with open('demographics.json') as demographics_data:
    ...
    for c in count:
        if c["State"] == state:
            if c["Population"]["Population per Square Mile"] > highest_population:
                highest_population = c["Population"]["Population per Square Mile"]
                counties = c["County"]
    return counties
